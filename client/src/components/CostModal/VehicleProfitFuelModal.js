...
export default function VehicleProfitFuelModal({ month, onClose }) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API}/fuel-vinh-khuc/by-month`, {
        params: {
          month,
        },
      });
      ...
      console.error("Lỗi lấy danh sách lợi nhuận:", error);

      alert(error.response?.data?.message || "Lỗi lấy danh sách lợi nhuận");
    } finally {
      setLoading(false);
    }
  };
  ...
  const totalFuel = data.reduce(
    (sum, item) => sum + Number(item.cpNhienLieu || 0),
    0,
  );
      ...
      >
        ...
        <div className="px-4 py-3 border-b bg-gray-50 flex gap-8 justify-between">
          ...
          <div>
            ...
            <span className="font-bold text-red-600">{formatMoney(totalFuel)}</span>
          </div>
        </div>
